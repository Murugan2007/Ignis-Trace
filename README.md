...
- **3D Enclosure CAD:** `/assets/ignis-trace-enclosure-v1.stl` *(3D-printable tree-mount design)*
...
- [x] Implement gas-triggered thermal sweep escalation on the STM32 side with timeout/recovery
- [x] Build 8×13 LED matrix status display logic (cascade state visualization)
- [x] Implement 433 MHz SX1276 LoRa 6-byte packet protocol with ACK and retry logic
- [x] Build ESP32 gateway (binary → JSON parsing) with local event queuing
- [x] Build Streamlit dashboard (live map + thermal angle + alert feed + explainability + feedback loop)
